(function () {
  ...
  function bindEvents() {
    ...
    $('btnExport').addEventListener('click', async () => {
      const payload = { exportedAt: new Date().toISOString(), categories, recurringItems, transactions };
      const blob = new Blob([JSON.stringify(payload, null, 2)], { type: 'application/json' });
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url; a.download = `ledger-backup-${D.todayISO()}.json`;
    ...
    $('fileInput').addEventListener('change', (e) => {
      ...
      reader.onload = async (ev) => {
        try {
          ...
          if (!Array.isArray(data.categories) || !Array.isArray(data.transactions)) throw new Error('Unrecognized backup format');
          if (!confirm('Importing will replace all data currently on this device with the contents of this backup file. This can\'t be undone. Continue?')) return;
          for (const storeName of ['categories', 'recurringItems', 'transactions']) {
            const existing = await D.getAll(storeName);
            for (const rec of existing) await D.del(storeName, rec.id);
          }
          for (const c of data.categories) await D.put('categories', c);
          for (const i of (data.recurringItems || [])) await D.put('recurringItems', i);
          for (const t of data.transactions) await D.put('transactions', t);
          await reloadData();
          renderAll();
          toast('Imported');
        } catch (err) {
          alert("Couldn't read that file. Please choose a backup exported from this app.");
        }
      };
